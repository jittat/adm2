# This modules bootstrap Django.
# Taken from http://www.djangosnippets.org/snippets/374/
import os
import sys
...
def find_settings_path(path):
    """
    Retrieve the path of the settings.py module for a django
    project. This can be passed into django_utils.bootstrap). You can
    pass in the __file__ variable, or an absolute path, and it will
    find the closest settings.py module.
    """
    ...
    parent_path = os.path.abspath(os.path.join(path, '..'))
    settings_path = os.path.join(path, 'adm2/settings.py')
